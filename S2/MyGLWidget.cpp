...
void MyGLWidget::creaBuffers ()
{
  ...
  glGenBuffers(1, &VBO1); //Nos asigna un VBO
  glBindBuffer(GL_ARRAY_BUFFER, VBO1); //Activa el VBO
  glBufferData(GL_ARRAY_BUFFER, sizeof(Colores), Colores, GL_STATIC_DRAW); //Indico el bloque de datos a pasar a la GPU
  // Activem l'atribut que farem servir per vèrtex (només el 0 en aquest cas)	
  glVertexAttribPointer(colorLoc, 3, GL_FLOAT, GL_FALSE, 0, 0); //Asignara 3 floats al vertexLoc(apuntador de la variable entrada)
  glEnableVertexAttribArray(colorLoc); //Activamos
