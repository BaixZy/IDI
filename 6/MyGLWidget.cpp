...
{ //Vuevl a llamar a initialize para cargar los nuevos shaders
...
void MyGLWidget::keyPressEvent(QKeyEvent* event) {
  makeCurrent();
  switch (event->key()) {
        case Qt::Key_K:  // Mover foco en dirección X-
            rr.x -= 0.1f;  // Cambia según el paso que desees
            glUniform3fv(posFoc, 1, &rr[0]);
            break;
        case Qt::Key_L:  // Mover foco en dirección X+
            rr.x += 0.1f;  // Cambia según el paso que desees
            glUniform3fv(posFoc, 1, &rr[0]);
            break;
